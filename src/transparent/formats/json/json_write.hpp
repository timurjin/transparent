...
template<typename T>
void write(const T& src, std::ostream* dst)
{
	transparent::formats::json::json_writer_visitor v(dst);
	transparent::utils::iterate(src, v);
}

template<typename T>
std::string write(const T& src)
{
	std::stringstream dst_;
	transparent::formats::json::json_writer_visitor v(&dst_);
	transparent::utils::iterate(src, v);
	return dst_.str();
}

template<typename T>
void write(const T& src, std::string* dst)
{
	std::stringstream dst_;
	transparent::formats::json::json_writer_visitor v(&dst_);
	transparent::utils::iterate(src, v);
